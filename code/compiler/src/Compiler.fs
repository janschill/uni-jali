...
let rec reduce2 (e: Expr) (context: bool) (store: Expr Env): Expr =
    ...
    let x =
        ...
        | List (list) ->
            ...
            then getValues reducedItems |> ListValue |> Constant
        ...
        | Pattern (matchExpression, patternList) ->
            ...
            | _ ->
                ...
                let rec match1 (actual: Expr, case: Expr) =
                    ...
                    | Constant (ADTValue (name, _, [])), Variable x when x = name -> StaticMatch []
                    | Constant (ADTValue (name, _, values)), Apply (Variable name', exprs) ->
                        if name = name' then matchMany (List.map Constant values) exprs else NoMatch
                    | Constant (ADTClosure ((name, _), _, [])), Variable x
                        when name = x -> StaticMatch []
                    | Variable dyn, Variable x ->
                        ...
                        | Some (Constant (ADTValue (name, sname, vals))) -> DynamicMatch <| collect case // the pattern variable might be an adtvalue, e.g. Increment, but matching would have happened in the previous statement
                    ...
                    | _, Variable x ->
                        ...
                        | v -> printfn "Unexpected value for %s -> %A" x  v ; NoMatch
                ...
                | ((_, body), true) :: _ ->
                    ...
                    body
                | [] -> raise <| ReduceError(e, "All patterns are known statically to not match")
